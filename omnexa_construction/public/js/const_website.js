	...
	const NAV_MEGA = [
		{
			...
			en: "About",
			items: [
				{ href: "/construction#const-colleges-section", ar: "الكليات والمدارس", en: "Colleges & Schools" },
		...
		},
		{
			...
			en: "Academics",
			items: [
				{ href: "/construction/programs", ar: "البرامج والتخصصات", en: "Programs & Majors" },
				{ href: "/construction#const-colleges-section", ar: "الكليات", en: "Colleges" },
				{ href: "/construction#const-scholarships-section", ar: "المنح الدراسية", en: "Scholarships" },
			],
		},
		{
			key: "admissions",
			...
			en: "Admissions",
			items: [
				{ href: "/construction/apply", ar: "التقديم الأونلاين", en: "Online Application" },
				{ href: "/construction#const-admission-band", ar: "خطوات القبول", en: "Admission Steps" },
				{ href: "/construction#const-international-section", ar: "الطلاب الدوليون", en: "International Students" },
			],
		},
		{
			key: "portals",
			...
			en: "Portals",
			items: [
				{ href: "__DESK__/construction-student-portal", ar: "بوابة الطالب", en: "Student Portal" },
				{ href: "__DESK__/construction-parent-mobile", ar: "بوابة ولي الأمر", en: "Parent Portal" },
				{ href: "__DESK__/construction-teacher-gradebook", ar: "بوابة أعضاء هيئة التدريس", en: "Faculty Portal" },
				{ href: "__DESK__/construction-workcenter", ar: "الإدارة", en: "Administration" },
			],
		},
	];
	...
	window.ConstSite = {
		...
		defaultConfig() {
			return {
				...
				tagline_en: "Build your future with a world-class construction institution",
				...
				urls: {
					...
					apply: "/construction/apply",
					desk: "/app/construction-workcenter",
					student_portal: "/app/construction-student-portal",
					parent_portal: "/app/construction-parent-mobile",
					faculty_portal: "/app/construction-teacher-gradebook",
		...
		t(key) {
			const map = {
				...
				cta_title: { ar: "جاهزون لرفع تجربة التعليم؟", en: "Ready to elevate construction?" },
				cta_sub: {
		...
		async loadConfig() {
			try {
				if (typeof frappe !== "undefined" && frappe.call) {
					const r = await frappe.call({
						method: "omnexa_construction.api.public_construction_site.get_site_config",
					});
					this.config = Object.assign(this.defaultConfig(), r.message || {});
				} else {
					const res = await fetch("/api/method/omnexa_construction.api.public_construction_site.get_site_config");
			...
			if (this.config.accent_color) {
				document.documentElement.style.setProperty("--const-teal", this.config.accent_color);
			}
			if (this.config.gold_color) {
				document.documentElement.style.setProperty("--const-gold", this.config.gold_color);
			}
		},
		...
		renderChrome() {
			...
			const nav = [
				...
				{ href: "/construction#const-colleges-section", ar: "الكليات", en: "Colleges", page: "" },
				{ href: "/construction/programs", key: "programs", page: "programs" },
				{ href: "/construction#const-gallery-section", ar: "الأنشطة", en: "Activities", page: "" },
				{ href: "/construction#const-news-section", ar: "الأخبار", en: "News", page: "" },
				{ href: "/construction/apply", key: "apply", page: "apply" },
			];
			...
			if (header) {
				header.innerHTML = `
					<div class="const-topbar"><div class="const-wrap const-topbar-inner">
						...
						<span>✉ admissions@omnexa.construction</span>
						<span class="const-topbar-links">
							<a href="${this.esc(this.deskPortalHref("construction-teacher-gradebook"))}">${this.lang === "ar" ? "أعضاء هيئة التدريس" : "Faculty"}</a>
							<a href="${this.esc(this.deskPortalHref("construction-student-portal"))}">${this.lang === "ar" ? "الطلاب" : "Students"}</a>
							<a href="${this.esc(this.deskPortalHref("construction-parent-mobile"))}">${this.lang === "ar" ? "أولياء الأمور" : "Parents"}</a>
						</span>
					</div></div>
					<div class="const-wrap const-header-inner">
						...
						<div class="const-actions">
							...
							<a class="const-btn const-btn-primary" href="/construction/apply">${this.t("apply_now")}</a>
						</div>
					</div>`;
				...
				footer.innerHTML = `
					<div class="const-wrap const-footer-grid const-footer-premium">
						<div>
							...
							<p class="const-footer-contact">📞 +966 11 000 0000 · ✉ admissions@omnexa.construction</p>
						</div>
						<div>
							<h4>${this.lang === "ar" ? "الكليات" : "Colleges"}</h4>
							<p><a href="/construction#const-colleges-section">${this.lang === "ar" ? "الطب والهندسة" : "Medicine & Engineering"}</a></p>
							<p><a href="/construction/programs">${this.t("programs")}</a></p>
							<p><a href="/construction#const-scholarships-section">${this.lang === "ar" ? "المنح" : "Scholarships"}</a></p>
						</div>
						<div>
							<h4>${this.lang === "ar" ? "القبول" : "Admissions"}</h4>
							<p><a href="/construction/apply">${this.t("apply_now")}</a></p>
							<p><a href="/construction#const-admission-band">${this.lang === "ar" ? "خطوات التقديم" : "How to Apply"}</a></p>
							<p><a href="/construction#const-international-section">${this.lang === "ar" ? "الطلاب الدوليون" : "International"}</a></p>
						</div>
						<div>
							<h4>${this.lang === "ar" ? "البوابات" : "Portals"}</h4>
							<p><a href="${this.esc(this.deskPortalHref("construction-student-portal"))}">${this.t("student_portal")}</a></p>
							<p><a href="${this.esc(this.deskPortalHref("construction-parent-mobile"))}">${this.t("parent_portal")}</a></p>
							<p><a href="${this.esc(this.deskPortalHref("construction-teacher-gradebook"))}">${this.lang === "ar" ? "هيئة التدريس" : "Faculty"}</a></p>
		...
		init_home() {
			...
			if (hero) {
				hero.innerHTML = `
					...
					<div class="const-wrap const-hero-premium-inner">
						<span class="const-eyebrow const-eyebrow-light">Omnexa Construction · World-Class</span>
						...
						<div class="const-hero-cta">
							<a class="const-btn const-btn-accent" href="/construction/apply">${this.lang === "ar" ? "ابدأ التقديم" : "Start Application"}</a>
							<a class="const-btn const-btn-ghost-light" href="/construction/programs">${this.lang === "ar" ? "استكشف البرامج" : "Explore Programs"}</a>
						</div>
			...
			if (trust) {
				const values = [
					{ icon: "🌍", ar: "تعليم عالمي", en: "Global Construction" },
			...
			this.renderColleges("const-colleges-section");
			...
			if (stats && cfg.stats) {
				...
				stats.innerHTML = `
					<div class="const-wrap const-stats-grid">
						<div><div class="const-stat-num">${s.institutions || 0}</div><div class="const-stat-label">${this.t("institutions")}</div></div>
						<div><div class="const-stat-num">${s.programs || 0}</div><div class="const-stat-label">${this.t("programs")}</div></div>
						<div><div class="const-stat-num">${s.students || 0}</div><div class="const-stat-label">${this.t("students")}</div></div>
						<div><div class="const-stat-num">${s.teachers || 0}</div><div class="const-stat-label">${this.t("teachers")}</div></div>
					</div>`;
			}

			const journey = document.getElementById("const-journey-section");
			if (journey) {
				...
				journey.innerHTML = `
					<div class="const-wrap">
						<div class="const-section-title">
							<span class="const-eyebrow">Student Lifecycle</span>
				...
				cta.innerHTML = `
					<div class="const-wrap">
						...
						<div class="const-cta-actions">
							<a class="const-btn const-btn-gold" href="/construction/apply">${this.t("apply_now")}</a>
							<a class="const-btn const-btn-ghost-light" href="${cfg.urls?.laravel_login || "https://kemetgate.com/login"}" target="_blank" rel="noopener">${this.t("login")}</a>
						</div>
					</div>`;
			}

			this.renderInstitutionTypes("const-institution-types");
			this.renderAdmissionBand("const-admission-band");
			this.renderServices("const-services-section");
			this.renderGallery("const-gallery-section");
			this.renderInternational("const-international-section");
			this.renderOnlineLearning("const-online-section");
			this.renderScholarships("const-scholarships-section");
			this.renderNews("const-news-section");
			this.renderPartners("const-partners-section");
			this.renderInstitutions("const-institutions");
		},
		...
		renderColleges(hostId) {
			...
			host.innerHTML = `
				<div class="const-wrap">
					<div class="const-section-title">
						<span class="const-eyebrow">Academic Excellence</span>
					...
					<div class="const-college-grid">
						${colleges
							...
							<div class="const-college-card">
								<div class="const-college-img"><img src="${this.esc(c.image)}" alt="" loading="lazy" onerror="this.src='/assets/omnexa_construction/construction.svg'" /></div>
								<div class="const-college-body">
									<h3>${this.esc(this.lang === "ar" ? c.name_ar : c.name_en)}</h3>
									<p>${c.programs || 0} ${this.lang === "ar" ? "برنامج" : "programs"}</p>
									<a class="const-btn const-btn-sm const-btn-primary" href="/construction/apply">${this.t("apply_now")}</a>
								</div>
							</div>`
							)
							.join("")}
					</div>
					<div class="const-section-cta"><a class="const-btn const-btn-outline" href="/construction/programs">${this.lang === "ar" ? "عرض جميع الكليات" : "View All Colleges"}</a></div>
				</div>`;
		},
		...
		renderAdmissionBand(hostId) {
			...
			host.innerHTML = `
				<div class="const-wrap">
					...
					<div class="const-admission-steps">
						${steps.map((s, i) => `<div class="const-adm-step"><span class="const-adm-icon">${s.icon}</span><span class="const-adm-num">${i + 1}</span><p>${s.t}</p></div>`).join("")}
					</div>
					<a class="const-btn const-btn-gold const-btn-lg" href="/construction/apply">${this.t("apply_now")}</a>
				</div>`;
		},
		...
		renderServices(hostId) {
			...
			host.innerHTML = `
				<div class="const-wrap">
					<div class="const-section-title">
						<span class="const-eyebrow">Student Services</span>
		...
		renderInternational(hostId) {
			...
			host.innerHTML = `
				<div class="const-wrap const-split-section">
					<div>
						<span class="const-eyebrow">Global Campus</span>
						<h2>${this.lang === "ar" ? "الطلاب الدوليون" : "International Students"}</h2>
						<ul class="const-check-list">${items.map((i) => `<li>${i}</li>`).join("")}</ul>
						<a class="const-btn const-btn-primary" href="/construction/apply">${this.t("apply_now")}</a>
					</div>
					<div class="const-split-img"><img src="https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=800&q=80" alt="" loading="lazy" /></div>
				</div>`;
		},
		...
		renderOnlineLearning(hostId) {
			...
			host.innerHTML = `
				<div class="const-wrap">
					<div class="const-section-title">
						<span class="const-eyebrow">Digital Learning</span>
						<h2>${this.lang === "ar" ? "التعلّم الإلكتروني" : "Online Learning"}</h2>
					</div>
					<div class="const-online-grid">
						${items.map((t) => `<div class="const-online-card">💻 ${t}</div>`).join("")}
					</div>
					<p class="const-center"><a href="${(this.config && this.config.urls && this.config.urls.laravel_portal) || "https://kemetgate.com"}" class="const-card-link" target="_blank" rel="noopener">kemetgate LMS →</a></p>
				</div>`;
		},
		...
		renderScholarships(hostId) {
			...
			host.innerHTML = `
				<div class="const-wrap">
					<div class="const-section-title">
						<span class="const-eyebrow">Scholarships</span>
						<h2>${this.lang === "ar" ? "المنح الدراسية" : "Scholarships"}</h2>
					</div>
					<div class="const-scholarship-grid">
						${rows
							.map(
								(s) => `
							<div class="const-scholarship-card">
		...
		renderPartners(hostId) {
			...
			const partners =
				this.lang === "ar"
					...
					: ["Ministry of Construction", "Industry Partners", "International Universities", "Microsoft", "Oracle", "AWS"];
			host.innerHTML = `
				<div class="const-wrap">
					<div class="const-section-title">
		...
		renderInstitutionTypes(hostId) {
			...
			if (typeof frappe !== "undefined" && frappe.call) {
				frappe.call({
					method: "omnexa_construction.api.public_construction_site.get_public_institution_types",
					callback: (r) => {
				...
				<div class="const-type-grid">
					${types
						.map((row) => {
							...
							const statusClass = active ? "const-status-active" : inactive ? "const-status-inactive" : "const-status-off";
							const statusLabel = active
								...
								: inactive
									...
									: this.lang === "ar"
										...
										: "Not active";
							return `
						<div class="const-type-card ${statusClass}">
							...
							<div class="const-type-body">
								<span class="const-type-icon">${typeIcons[row.institution_type] || "🏢"}</span>
								<span class="const-status-pill ${statusClass}">${statusLabel}</span>
								<h3>${this.esc(row.name)}</h3>
								<p class="const-muted">${this.esc(row.institution_type)}${row.academy_type ? " · " + this.esc(row.academy_type) : ""}</p>
								<div class="const-type-stats">
									<span>👨‍🎓 ${row.students || 0}</span>
									<span>👩‍🏫 ${row.teachers || 0}</span>
									<span>📋 ${row.applications || 0}</span>
								</div>
								${row.institution ? `<a class="const-card-link" href="/construction/apply?institution=${encodeURIComponent(row.institution)}">${this.t("apply_now")} →</a>` : ""}
							</div>
						</div>`;
						})
						.join("")}
		...
		renderGallery(hostId) {
			...
			host.innerHTML = `
				<div class="const-wrap">
					<div class="const-section-title">
						<span class="const-eyebrow">Campus Life</span>
		...
		async renderInstitutions(hostId) {
			...
			const r = await frappe.call({
				method: "omnexa_construction.api.public_construction_site.get_public_institutions",
			});
			const rows = r.message || [];
			host.innerHTML = `
				<div class="const-card-grid">
					${rows
						.map(
							(row) => `
						<div class="const-card const-card-rich">
							<div class="const-card-img"><img src="https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=600&q=70" alt="" loading="lazy" /></div>
							<div class="const-card-body">
							<span class="const-badge">${this.esc(row.institution_type || "")}</span>
							<h3>${this.esc(row.institution_name)}</h3>
							<p>${this.esc(row.city || "")}</p>
							<a class="const-card-link" href="/construction/apply?institution=${encodeURIComponent(row.name)}">${this.t("apply_now")} →</a>
							</div>
						</div>`
						)
		...
		async init_programs() {
			const filterHost = document.getElementById("const-program-filters");
			const host = document.getElementById("const-programs-list");
			if (!host) return;
			...
			try {
				...
				if (typeof frappe !== "undefined" && frappe.call) {
					const r = await frappe.call({
						method: "omnexa_construction.api.public_construction_site.get_public_programs",
					});
					rows = r.message || [];
				} else {
					const res = await fetch("/api/method/omnexa_construction.api.public_construction_site.get_public_programs");
					const data = await res.json();
					rows = data.message || [];
				}
			...
			if (filterHost) {
				filterHost.innerHTML = `
					<button type="button" class="const-filter-btn active" data-filter="all">${this.t("all_programs")}</button>
					${types
						.map((t) => `<button type="button" class="const-filter-btn" data-filter="${this.esc(t)}">${this.esc(t)}</button>`)
						.join("")}`;
				filterHost.querySelectorAll(".const-filter-btn").forEach((btn) => {
					btn.addEventListener("click", () => {
						filterHost.querySelectorAll(".const-filter-btn").forEach((b) => b.classList.remove("active"));
				...
				<div class="const-card-grid">
					${rows
						...
						<div class="const-card">
							<span class="const-badge">${this.esc(row.program_type || row.degree_level || "")}</span>
							...
							<a class="const-card-link" href="/construction/apply?institution=${encodeURIComponent(row.institution || "")}">${this.t("apply_now")} →</a>
						</div>`
						)
						.join("") || `<p style="text-align:center">—</p>`}
				</div>`;
		...
		init_apply() {
			const aside = document.getElementById("const-apply-aside");
			if (aside) {
				const steps =
					this.lang === "ar"
						? [
			...
			const host = document.getElementById("const-apply-form-host");
			if (!host) return;
			const params = new URLSearchParams(window.location.search);
			const preInst = params.get("institution");

			frappe.call({
				method: "omnexa_construction.api.construction_admissions.get_public_apply_context",
				callback: (r) => {
					const ctx = r.message || {};
					...
					host.innerHTML = `
						<form id="const-apply-form">
							<div class="const-form-group"><label>${this.t("applicant")} *</label>
								<input type="text" name="applicant_name" required autocomplete="name" /></div>
							<div class="const-form-group"><label>${this.t("institution")} *</label>
								<select name="institution" required><option value="">${this.t("select")}</option>${instOpts}</select></div>
							<div class="const-form-group"><label>${this.t("academic_year")} *</label>
								<select name="academic_year" required><option value="">${this.t("select")}</option></select></div>
							<div class="const-form-group"><label>${this.t("grade")}</label>
								<input type="text" name="grade_level" /></div>
							<div class="const-form-group"><label>${this.t("guardian_email")}</label>
								<input type="email" name="guardian_email" autocomplete="email" /></div>
							<button type="submit" class="const-btn const-btn-primary" style="width:100%;margin-top:12px;padding:14px">${this.t("submit")}</button>
						</form>
						<div id="const-apply-result" style="margin-top:16px"></div>`;
					...
					host.querySelector("#const-apply-form").addEventListener("submit", (e) => {
						e.preventDefault();
						const fd = Object.fromEntries(new FormData(e.target).entries());
						frappe.call({
							method: "omnexa_construction.api.construction_admissions.submit_online_application",
							args: fd,
							callback(res) {
								document.getElementById("const-apply-result").innerHTML = `<div style="color:#0d6e3a;padding:16px;background:#ecfdf5;border-radius:12px;border:1px solid #a7f3d0">${ConstSite.t("success")}: <strong>${ConstSite.esc(res.message.application)}</strong></div>`;
							},
							error(err) {
								document.getElementById("const-apply-result").innerHTML = `<div style="color:#c62828;padding:12px">${ConstSite.esc(err.message || "Error")}</div>`;
							},
						});
					});
				},
